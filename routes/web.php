...
// Kijelentkezés //
Route::post('/kijelentkezes', function () {
    session()->flush();
    return redirect('/');
})->name('kijelentkezes');
...
Route::post('/regisztracio', function (Request $request) {
    ...
    $existingCheck = DB::table('felhasznalok')->where('felhasznalonev', $validatedData['felhasznalonev'])->first();

    if ($existingCheck) {
        return redirect()->back()->with('error', 'Már van ilyen felhasználó.');
    }

    // Új felhasználó beszúrása
    DB::table('felhasznalok')->insert([
        'felhasznalonev' => $validatedData['felhasznalonev'],
        'szerep' => $validatedData['szerep'],
